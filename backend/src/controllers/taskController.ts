...
const processedReorderOps = new Map<string, number>();
const OPERATION_TTL = 5000;

const isDuplicateOp = (operationId: string) => {
  const now = Date.now();
  if (processedReorderOps.has(operationId)) return true;
  processedReorderOps.set(operationId, now);
  setTimeout(() => processedReorderOps.delete(operationId), OPERATION_TTL);
  return false;
};
...
const getAndBroadcastTasks = async (projectId: string) => {
  ...
  getIO().to(projectId).emit("tasks:updated", tasks);
};
...
export const createTask = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (assigneeEmail) {
      ...
      if (!assignee)
        return next(new AppError("Assignee with that email not found", 404));
...
export const updateTask = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const updatedTask = await Task.findByIdAndUpdate(taskId, filteredBody, {
      new: true,
    }).populate("assignee", "name email avatarUrl");
...
export const reorderTasks = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (operationId && isDuplicateOp(operationId)) {
      return sendSuccess(res, null, "Duplicate reorder ignored");
    }
