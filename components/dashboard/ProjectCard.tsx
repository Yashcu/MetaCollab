...
export function ProjectCard({ project }: ProjectCardProps) {
    // BUG FIX: GET /api/projects uses `select` with `_count: { select: { members, tasks } }`
    // so project.members is NOT an array here — it's undefined on the dashboard list.
    // project._count.members is the correct field to read member count from.
    // project.members only exists on the single-project GET (/api/projects/[id]) which uses `include`.
    const memberCount = (project as unknown as { _count?: { members: number } })._count?.members
